...
describe('Post Hoax', () => {
  ...
  `(
    'returns a message of $message when a hoax is submitted without authorisation in language $language',
  ...
  `(
    'returns a successful message of $message when a hoax is save when in language $language',
    async ({ language, message }) => {
      await addUser();
      const hoax = await postHoax(
        { content: 'Hoax content' },
  ...
  `(
    'returns a 400 and message of $message when a hoax is less than 10 characters in lenght and language is $language',
    async ({ language, message }) => {
      await addUser();
      const hoax = await postHoax(
        { content: '123456789' },
  ...
  `(
    'returns $message when hoax content is not between 10 and 5000 characters and the language is $is',
  ...
  it('associates hoax with attachement in the database', async () => {
    const uploadedFile = await uploadFile();
    const uploadedFileId = uploadedFile.body.id;
    await addUser();
    await postHoax(
      {
        content: 'Hoax content',
    ...
    const fileAttachmentInDB = await FileAttachment.findOne({
      where: { id: uploadedFileId },
    });
    expect(fileAttachmentInDB.hoaxId).toBe(hoax.id);
  });

  it('returns 200 even if the attachement does not exist', async () => {
    await addUser();
    const response = await postHoax({ content: 'Hoax content', fileAttachment: 1000 }, { auth: credentials });
    expect(response.status).toBe(200);
  });

  it('the old association is kept when a new hoax is submitted with an old attachement', async () => {
    const uploadedFile = await uploadFile();
    const uploadedFileId = uploadedFile.body.id;
    await addUser();
    await postHoax(
    ...
    const attachmentAfterSecond = await FileAttachment.findOne({
      where: { id: uploadedFileId },
    });

    expect(attachment.hoaxId).toBe(attachmentAfterSecond.hoaxId);
  });
});
